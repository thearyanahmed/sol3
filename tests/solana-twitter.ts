...
describe("solana-twitter", () => {
  // Configure the client to use the local cluster.
    ...
    it("can send a new tweet", async () => {
        ...
        await program.rpc.sendTweet(t.topic,t.content, {
            accounts: {
        ...
        assert.equal(tweetAccount.topic, t.topic);
        assert.equal(tweetAccount.content, t.content);
    ...
    it('can send a new tweet from a different author', async () => {
        ...
        const tweet = anchor.web3.Keypair.generate();
        await program.rpc.sendTweet('welcome', 'Yay Tofu!', {
            accounts: {
                tweet: tweet.publicKey,
        ...
        assert.equal(tweetAccount.topic, 'welcome');
    ...
    it('cannot provide a content with more than 280 characters', async () => {
        try {
            ...
            await program.rpc.sendTweet('welcome', contentWith281Chars, {
                accounts: {
                    tweet: tweet.publicKey,
                    author: program.provider.wallet.publicKey,
                    systemProgram: anchor.web3.SystemProgram.programId,
        ...
        const tweetAccounts = await program.account.tweet.all([
            {
                memcmp: {
                    ...
                    bytes: bs58.encode(Buffer.from('welcome')),
                }
            }
        ]);
